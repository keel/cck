...
/**
 * 获取某个时间当月的天数
 * @param  {int} msTime 毫秒数
 * @return {int}        天数
 */
const dayCountOfMonth = function(msTime) {
...
/**
 *
 * 获取YYYYMMDD'T'HHMMSS'Z形式的UTC时间
 *
 */
const getUTCTime = function(t = new Date()) {
  const year = t.getUTCFullYear();
  const month = twoInt(t.getUTCMonth() + 1); // 月份从0开始，需要加1
  const day = twoInt(t.getUTCDate());
  const hours = twoInt(t.getUTCHours());
  const minutes = twoInt(t.getUTCMinutes());
  const seconds = twoInt(t.getUTCSeconds());
 ...
 * @return {string}
 */
const timeFormatUTC = function(formatStr, millSec) {
  const d = millSec ? new Date(millSec) : new Date();
  const df = {
    'YYYY': d.getUTCFullYear(),
    'yyyy': d.getUTCFullYear(),
    'MM': twoInt(d.getUTCMonth() + 1),
    // 'DD': twoInt(d.getDate()),//年中的天数
    'dd': twoInt(d.getUTCDate()),
    'HH': twoInt(d.getUTCHours()),
    // 'hh': (d.getHours() < 12) ? twoInt(d.getHours()) : twoInt(d.getHours() - 12),//12小时制
    'mm': twoInt(d.getUTCMinutes()),
    'ss': twoInt(d.getUTCSeconds()),
    'SSS': threeInt(d.getUTCMilliseconds()),
  };
  if (!formatStr) {
...
//通过预生成数组提升zeroPadding性能
const zeroArr = ['0'];
const zeroPadMaxLen = 101;
for (let i = 1, len = zeroPadMaxLen; i < len; i++) {
  let zeroStr = '';
  for (let j = 0; j < i; j++) {
    zeroStr += '0';
  }
  zeroArr.push(zeroStr);
}

//高性能zeroPadding,intLen最高支持zeroPadMaxLen位
const zeroPadding = function(int, intLen) {
  const cLen = ('' + int).length;
  const needPad = intLen - cLen;
  if (needPad > 0) {
    const padStr = zeroArr[needPad];
    return padStr + int;
  }
  return '' + int;
};
...
exports.priceIntShow = priceIntShow;
...
exports.dayCountOfMonth = dayCountOfMonth;
exports.getUTCTime = getUTCTime;
exports.timeFormatUTC = timeFormatUTC;